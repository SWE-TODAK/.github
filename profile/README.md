...
### ⏳ 진료 대기의 비효율성
아픈 몸을 이끌고 병원을 방문해도, 기약 없는 대기 시간은 환자에게 큰 피로감을 줍니다. 또한 내 증상에 맞는 진료과나 원하는 의사에게 진료를 받기 위해 일일이 확인해야 하는 번거로움이 존재합니다.

### 💡 토닥의 해결책
토닥은 이러한 문제를 해결하기 위해 ‘진료 음성 녹음 → 텍스트 변환(STT) → AI 요약 → 기록 관리’로 이어지는 헬스케어 기록 자동화 프로세스를 구축했습니다.
더불어, 맞춤형 예약 시스템을 도입하여 사용자가 이비인후과, 소아과 등 필요한 진료과와 의료진을 직접 선택하고 예약할 수 있게 했습니다. 이를 통해 불필요한 대기 시간을 없애고, 환자가 자신의 진료 내용과 건강 데이터를 주도적으로 관리하는 통합 의료 경험을 제공합니다.
